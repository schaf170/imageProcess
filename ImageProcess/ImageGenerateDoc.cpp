...
BEGIN_MESSAGE_MAP(CImageGenerateDoc, CDocument)
	...
	ON_COMMAND(ID_GENERATE_FILLTURQUOISE, &CImageGenerateDoc::OnGenerateFillturquoise)
...
void CImageGenerateDoc::OnGenerateFillturquoise()
{
	for (int r = 0; r < m_image.GetHeight(); r++)
	{
		// Looping over the columns of the image
...
void CImageGenerateDoc::OnGenerateDiagonalgradient()
{
	for (int r = 0; r < m_image.GetHeight(); r++)
	{
		...
		{
			m_image[r][c * 3 + 0] = 0;
			m_image[r][c * 3 + 1] = (BYTE(float(r) / float(m_image.GetHeight() - 1) * 255) + BYTE(float(c) / float(m_image.GetWidth() - 1) * 255)) / 2;
			m_image[r][c * 3 + 2] = 255 - (BYTE(float(r) / float(m_image.GetHeight() - 1) * 255) + BYTE(float(c) / float(m_image.GetWidth() - 1) * 255)) / 2;
		}
	}
