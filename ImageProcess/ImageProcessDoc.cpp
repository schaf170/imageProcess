...
void CImageProcessDoc::OnFilterMonochrome()
{
	...
	for (int r = 0; r < m_image2.GetHeight(); r++)
	{
		for (int c = 0; c < m_image2.GetWidth(); c++)
		{
			average = (m_image1[r][c * 3] + m_image1[r][c * 3 + 1] + m_image1[r][c * 3 + 2]) / 3;
...
{
	//Note: this process takes about a minute to finish. It is not broken. Give it the time it needs and it will run correctly. As for ideas to speed it up. I'm all ears.
	BeginWaitCursor();
	m_image2.SetSameSize(m_image1);
			...
			for (int color = 0; color < 3; color++)
			{
				...
				for (int i = -1; i <= 1; i++)
				{
					if ((r + i) >= 0 && (r + i) < m_image2.GetHeight())
					{
						...
						{
							if ((c + j) >= 0 && (c + j) < m_image2.GetWidth() * 3)
								pixels.push_back(m_image1[r + i][(c + j) * 3 + color]);
						}
				...
				m_image2[r][c * 3 + color] = BYTE(median);
				pixels.clear();
			}
		}
